<x-admin.layout title="Penilaian: {{ $tugasAkhir->title }}">

<div x-data="penilaianTable({
    items: {{ Js::from($items) }},
    tugasId: {{ $tugasAkhir->id }},
    updateUrlPattern: '{{ route('admin.nilai.update', ':id') }}',
    exportUrl: '{{ route('admin.tugas-akhir.export-nilai', $tugasAkhir) }}',
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 mb-1">
                <a href="{{ route('admin.nilai.index') }}"
                   class="inline-flex items-center gap-1 text-xs font-medium text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
                    </svg>
                    <span>Kembali ke Rekap Nilai Jurusan</span>
                </a>
            </div>
        ...
        <div class="flex items-center gap-3">
            <button type="button"
                    @click="triggerExport()"
                    class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-medium rounded-lg shadow-xs transition-colors">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 p-4 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-xs">
        ...
        <div class="border-r border-gray-200 dark:border-gray-800 last:border-0 pr-3">
            ...
            <span class="text-lg font-bold text-indigo-600 dark:text-indigo-400" x-text="dinilaiCount"></span>
        </div>
        <div class="border-r border-gray-200 dark:border-gray-800 last:border-0 pr-3">
        ...
        <div>
            ...
            <span class="text-lg font-bold text-cyan-600 dark:text-cyan-400" x-text="averageScore"></span>
        </div>
    </div>
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                            ...
                            <td class="px-4 py-3 text-center">
                                <template x-if="item.is_completed">
                                    <input type="number"
                                           ...
                                           placeholder="1-100"
                                           class="w-24 text-center px-2.5 py-1.5 text-sm font-semibold rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                                </template>
                                <template x-if="!item.is_completed">
                            ...
                            <td class="px-4 py-3">
                                <input type="text"
                                       ...
                                       class="w-full px-3 py-1.5 text-xs rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                            </td>
