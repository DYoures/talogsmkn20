        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-800 text-left">
                    ...
                    <th class="px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Aksen</th>
            ...
            <tbody class="divide-y divide-gray-800">
                ...
                <tr class="hover:bg-gray-800/50 transition-colors" x-data="{ confirmDelete: false }">
                    <td class="px-5 py-3.5 font-medium text-white">
                        <div class="flex items-center gap-2.5">
                            <span class="w-3 h-3 rounded-full shrink-0 border border-white/20"
                                  style="background-color: {{ $jurusan->accent_color }}; box-shadow: 0 0 6px {{ $jurusan->accent_color }}80;"></span>
                            <span>{{ $jurusan->name }}</span>
                            @if($jurusan->kode)
                                <span class="px-1.5 py-0.5 rounded text-[10px] font-mono font-bold"
                                      style="color: {{ $jurusan->accent_color }}; background-color: {{ $jurusan->accentRgba(0.12) }}; border: 1px solid {{ $jurusan->accentRgba(0.3) }};">
                                    {{ $jurusan->kode }}
                                </span>
                            @endif
                        </div>
                    ...
                    <td class="px-5 py-3.5">
                        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-mono"
                              style="color: {{ $jurusan->accent_color }}; background-color: {{ $jurusan->accentRgba(0.1) }};">
